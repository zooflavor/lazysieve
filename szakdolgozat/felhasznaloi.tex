...
%%% EMIL: ez a "program egy külön része" az amit az első paragrafusban írtál le? Ha igen kicsit kusza a dolog.
...
%%% EMIL: Hasonlóan, ez az amit egy bekezdéssel korábban emlegetsz, vagy egy harmadik dolog? Ez egy triviális kérdés, nyilván ha valaki megnézi a kódot, látja rögtön, de írd is le a dolgozatba, akár mennyire trivinek is tűnik.

\section{Felhasznált módszerek}

%%% EMIL: Tudomásom szerint, ilyenkor azt illik csinálni hogy egy-egy új fogalom, elnevezés bevezetésénél, használsz pl idézőjeleket ``init'', ``generator'' és/vagy még ezeket dőlt betűkkel írod (az idézőjeleket csak első alkalommal kell kiírni).  Illetve itt talán a \texttt{init} és \texttt{} ha ezek a futtatható állományok nevei.
...
%%% EMIL: Hááát... a prím szita egy intervallumon dönti el a számokról hogy prímek-e vagy sem, nem random lista prímjeiről... (pontosaban egy a*i + b alakú számok listájára is alkalmazható, ahol i egy intervallum beli).
...
\For{$i \gets 2$, $i \le n$, $i \gets i+1$}
	\If{$i$ nincs megjelölve}
                %%% EMIL: Ezt a ciklust $i^{2}$-től szokták indítani
...
%%% EMIL: "minden prímet sorba vesz"??? Ha már meg van neki az összes prím, akkor miért kellene szitálni... pro tip, olvasd újra és újra és újra a dolgozatot.
...
A C nyelv választását a hatékony végrehajtás és memóriafelhasználás, és  %%% EMIL: "és" szóismétlés?
a hordozhatóság indokolja, a generátor elkülönítését az automatizálhatóság indokolja.

%%% EMLI: generator vagy prímgenerátor?
...
%%% EMIL: A következő mondatot nem biztos hogy teljesen érthető.
...
%%% EMIL: Serious WTF mondat a következő? Milyen adatbázis???  Továbbá felhívnám a figyelmed arra hogy a következő 4 bekezdésből 3 "A program.." szavakkal kezdődik... az egyeten kivétel az "A gui" és szerintem most váltok át "átfutó" üzemmódba, azaz mostantól próbálok apróságokba nem belekötni, és azt tanácsolom hogy minimum 3x de inkább 5x olvasd majd át dolgot amikor úgy gondolod hogy kezd kész lenni... utána még számíts összvissz 20 újraolvasásra... a kisebb javítások/modosítások miatt.
...
%%% EMIL: nagyobb pontoságg mit takar egész pontosan?
...
%%% EMIL: szerintem ne itt kezd magyarazni a prioritásos soros szitát 2 sorban, mert nem lesz érthető. Vezesd be, mond meg mi a neve, írd le hogy a saját ötleted... de ne bróbálj egy másfél oldalas leírást másfél sorba sűríteni.
...
\end{lstlisting}
Ennek eredmény 4 bittérképfájl, amit a db mappába ment.  %%% EMIL: bitvektort tartalmazó bináris fájl!

%%% EMIL: többféleképpen egy szó
...
%%% EMIL: Nem ''idézőjel'' hanem ``idézőjel''
...
%%% EMIL: A pszeudoprím tesztről... ez itt jó hogy leírtad hogy "alapvetően más" módszerrel is szeretnél ellenőrizni. Kicsit "out of place"-nek mondanám a pszeudoprím tesztet, mert az prím teszt, ez meg szitálás... szitálással egy csomó számra csinálsz prímtesztet, a prímtesztnél meg csak egy szamról döntöd el... nyilván ha a "csomó szám" az csak egy akkor a szita prímteszt lesz, meg ha sok számot tesztelsz akkor olyan mintha szintálnál... de erőltetettnek tűnhet... illetve nem tom ezt hogy csináltad (és lusta vagyok most kódot nézni), egy kis intervallumon mindent pszeudoprímtesztelsz? vagy véletlenszerűen választasz egy egy számot és megnézed hogy a szita és ppt is ugyan azt mondja-e róluk? Én az utóbbit javasolnám, de bármelyiket csináltad, azt írd le, mert nem teljesen világos egész pontosan hogy ellenőrzöd a dolgokat pptvel. 

%%% EMIL: Azok a franya idezojelek. Javaslom a szoveg szerkesztok search and replace funkciojat.
...
%%% EMIL: A mintákat definiáltad micsodák, ilyen mintákról beszélünk?  
...
%%% EMIL: a módszerének komponensei? he? (a ... módszer rövid leírása a következő:)
...
%%% EMIL: milyen az hogy speed kezdetű könyvtás? Egy könyvtár nem igazán kezdődik... ugye? a neve esetleg... nyilván értem mire gondolsz, de néha az ilyen fogalmazások összezavarják az embert.
