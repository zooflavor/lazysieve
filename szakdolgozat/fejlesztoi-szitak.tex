...
%%% E: Mik az elmek (vagy ez itt meg nem fontos... esetleg valami ilyesmi "az edenysor elemei prim-pozicio parok lesznek az edenysoros szitalo algoritmusaban")
...
%%% E: Nekem ez itt tovabbra is nagyon boki a csoromet, hogy a p() mert mukodik egy edenysorra es egy elemre is... bar mostmar asszem ertem a jeloles szepseget
...
%%% E: erre nem hivatkozol sehol, pedig jo lenne.
\begin{algorithm}[H]
\floatname{algorithm}{Algoritmus}
...
%%% E: Az elozo mondat nem lenne jobb ide csatolni, ehhez a paragarafushoz.
%%% E: Egy kicst atirni a kovi mondatot? szamok amik sorpoziciora lepnek? bakanccsal?... ne inkam primek azok a szamok, melyek megegyeznek a sorpozicioikkal amelyeknek novelese nem tavolit el elemeket az edenysorbol. Es ebben a paragrafusban kellene megmondani hogy akkor mik is valojaban az elemek. 
...
%%% E: Noveleseert felelos algoritmus
...
%%% E: ide illene egy: "A helyesseg bizonyitasa a kovetkezo ket allitas bizonyitasabol all", nem?
\begin{equation}
\label{ures}
...
%%% E: ez igy miert is ellentmondas? mert egy a tavolsag kozbe meg ket < van? remelem ertheto mi a bajom, a vala < ... < valami + 1 nem tul ellentmondasoskinezetu
\begin{align*}
...
Ha $k' = k$, akkor
%%% E: ezt nem konnyu latni miert teljesul.
\begin{align*}
...
\begin{align*}
p(q)_{k'} &= (p(q)+1)_{k'} \\
  %%% E: A kozepso egyenletet se nem feltetlenul ertem miert van. miert lesz h(p(q) + 1, p(r)) = k'???
